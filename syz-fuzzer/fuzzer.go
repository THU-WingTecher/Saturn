...
func main() {
	...
	// init procUsage
	procUsage := make(map[int]bool, *flagProcs)
	...
	// open probe program
	// TODO: embed probe program into the binary
	probe_cmd := exec.Command("/root/kprobe")
	probe_cmd.Start()

	// open a thread to read dev mappings
	go fuzzer.readTracePipe()
...
// TODO: update corpus resource type
...
func (fuzzer *Fuzzer) readTracePipee() {
	r, _ := regexp.Compile(`KPROBE EXIT: filename = (.*), addr = (.*), addr name = (.*)\+(.*)`)
	log.Logf(0, "reading trace pipe(read kprobe output)...")

	// fixed file path for now
	...
	buf := make([]byte, 1024)
	for {
		log.Logf(0, "reading trace pipe...")
		n, _ := f.Read(buf)
		log.Logf(0, "log kprobe result %v", buf[:n])

		match := r.FindAllString(string(buf[:n]), -1)
		for _, m := range match {
			for dev := range fuzzer.devMap {
				if fuzzer.devMap[dev] == "" && strings.Contains(m, path.Base(dev)) {
					match_sub := r.FindStringSubmatch(m)
					if match_sub != nil {
						fuzzer.devMap[dev] = match_sub[3]
					}
				}
			}
		}
...
func (fuzzer *Fuzzer) readTracePipe() {
	r, _ := regexp.Compile(`KPROBE EXIT: filename = (.*), full path = (.*), addr = (.*), addr name = (.*)`)
	log.Logf(0, "reading trace pipe(read kprobe output)...")
	...
	for {
		...
		case event := <-channel:
			for dev := range fuzzer.devMap {
				if strings.Contains(event.Message, dev) {
					match_sub := r.FindStringSubmatch(event.Message)
					if match_sub != nil {
						fuzzer.devMap[dev] = match_sub[4][:len(match_sub[4])-3]
					}
				}
			}
		case err := <-errorChannel:
			for dev := range fuzzer.devMap {
				if strings.Contains(err.Error(), dev) {
					match_sub := r.FindStringSubmatch(err.Error())
					if match_sub != nil {
						fuzzer.devMap[dev] = match_sub[4][:len(match_sub[4])-3]
