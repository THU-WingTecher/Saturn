...
// fix the value of arg 'uid' to specified value
func (p *Prog) FixUidArg(uid int) {
	for _, c := range p.Calls {
		if c.Meta.CallName != "syz_attach_gadget" {
			continue
		}
		for idx, arg := range c.Meta.Args {
			if arg.Name == "uid" {
				c.Args[idx].(*ConstArg).Val = uint64(uid)
			}
		}
	}
